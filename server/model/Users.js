...
module.exports = {
  ...
  readUser: async (id) => {
    return db.query(`SELECT * FROM "user" ${id ? `WHERE id = ${id}` : ''}`);
  },

  // should maybe not send all columns, depends on UI
  readOrgUsers: async (orgId) => {
    return db.query(`SELECT * FROM "user" WHERE organization_id = ${orgId}`);
  },
  ...
  updateUser: async (id, body) => {
    ...
    const query = `INSERT "user" SET ${setStr} WHERE "user".id = ${db.escape(id)}`;
